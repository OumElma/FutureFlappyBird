...
# Load images
bg2 = pygame.image.load('images/bg.png')
...
class Pipe(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.image = pygame.transform.scale(self.image, (self.image.get_width(), self.image.get_height() // 2))
        self.rect = self.image.get_rect()
...
while run:
    ...
    # Check if bird has hit the ground (no game over condition)
    ...
    if not game_over and flying:
        ...
        if time_now - last_pipe > pipe_frequency:
            pipe_height = random.randint(300, 400)
            btm_pipe = Pipe(screen_width, screen_height - pipe_height - ground_img.get_height())
